...
function ttf2woff(arr, options) {
  ...
  for (i = 0; i < numTables; ++i) {
    ...
    if (String.fromCharCode.apply(null, tableEntry.Tag) !== "head") {
      var algntable = arr.subarray(
        tableEntry.Offset,
        tableEntry.Offset + longAlign(tableEntry.Length)
      );

      if (calc_checksum(algntable) !== tableEntry.checkSum) {
        throw (
          "Checksum error in " + String.fromCharCode.apply(null, tableEntry.Tag)
        );
      }
    }

    tableBuf.writeUint32BE(
      tableEntry.Tag.readUint32BE(0),
      i * SIZEOF.WOFF_ENTRY + WOFF_ENTRY_OFFSET.TAG
    );
    tableBuf.writeUint32BE(
      tableEntry.Length,
      i * SIZEOF.WOFF_ENTRY + WOFF_ENTRY_OFFSET.LENGTH
    );
    tableBuf.writeUint32BE(
      tableEntry.checkSum,
      i * SIZEOF.WOFF_ENTRY + WOFF_ENTRY_OFFSET.CHECKSUM
    );
    sfntSize += longAlign(tableEntry.Length);
  }

  var sfntOffset =
    SIZEOF.SFNT_HEADER + entries.length * SIZEOF.SFNT_TABLE_ENTRY;
  ...
  for (i = 0; i < entries.length; ++i) {
    ...
    var sfntData = arr.subarray(
      tableEntry.Offset,
      tableEntry.Offset + tableEntry.Length
    );
    ...
    if (String.fromCharCode.apply(null, tableEntry.Tag) === "head") {
      ...
      sfntData.writeUint32BE(
        checksumAdjustment,
        SFNT_ENTRY_OFFSET.CHECKSUM_ADJUSTMENT
      );
    }
    ...
    tableBuf.writeUint32BE(
      offset,
      i * SIZEOF.WOFF_ENTRY + WOFF_ENTRY_OFFSET.OFFSET
    );
    ...
    tableBuf.writeUint32BE(
      compLength,
      i * SIZEOF.WOFF_ENTRY + WOFF_ENTRY_OFFSET.COMPR_LENGTH
    );
